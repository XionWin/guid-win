...
public static class GLNanoVG
{
    ...
    static void glnvg__dumpProgramError(uint prog, string name)
    {
        ...
        int len = 0;
        GL.GetShaderInfoLog((int)prog, 512, out len, out sb);
    ...
    static int glnvg__renderCreateTexture2(object uptr, int type, int w, int h, int imageFlags, Bitmap bmp)
    {
        ...
        if (type == (int)NVGtexture.NVG_TEXTURE_RGBA)
            GL.TexImage2D(TextureTarget2d.Texture2D, 0, TextureComponentCount.Rgba, w, h, 0,
                OpenTK.Graphics.ES30.PixelFormat.Rgba, PixelType.UnsignedByte, data.Scan0);
        else
            //glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, w, h, 0, GL_RED, GL_UNSIGNED_BYTE, data);
            GL.TexImage2D(TextureTarget2d.Texture2D, 0, TextureComponentCount.Rgb, w, h, 0,
                OpenTK.Graphics.ES30.PixelFormat.Red, PixelType.UnsignedByte, data.Scan0);
    ...
    static int glnvg__renderCreateTexture(object uptr, int type, int w, int h, int imageFlags, byte[] data)
    {
        ...
        if (type == (int)NVGtexture.NVG_TEXTURE_RGBA)
            GL.TexImage2D(TextureTarget2d.Texture2D, 0, TextureComponentCount.Rgba, w, h, 0,
                OpenTK.Graphics.ES30.PixelFormat.Rgba, PixelType.UnsignedByte, data);
        else
            //glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, w, h, 0, GL_RED, GL_UNSIGNED_BYTE, data);
            GL.TexImage2D(TextureTarget2d.Texture2D, 0, TextureComponentCount.Rgb, w, h, 0,
                OpenTK.Graphics.ES30.PixelFormat.Red, PixelType.UnsignedByte, data);
    ...
    public static int glnvg__renderUpdateTexture(object uptr, int image, int x, int y, int w, int h, byte[] data)
    {
        ...
        if (tex.type == (int)NVGtexture.NVG_TEXTURE_RGBA)
            ...
            GL.TexSubImage2D(TextureTarget2d.Texture2D, 0, x, y, w, h,
                OpenTK.Graphics.ES30.PixelFormat.Rgba, PixelType.UnsignedByte, data);
        else
#if NANOVG_GLES2
    ...
    static void glnvg__renderFlush(object uptr, NVGcompositeOperationState compositeOperation)
    {
        ...
        if (gl.ncalls > 0)
        {
            ...
            glnvg__checkError(gl, "stroke fill 0-0");
            GL.BindBuffer(BufferTarget.ArrayBuffer, gl.vertBuf);
            glnvg__checkError(gl, "stroke fill 0-1");
            ...
            glnvg__checkError(gl, "stroke fill 0-2");

            GL.EnableVertexAttribArray(0);
            glnvg__checkError(gl, "stroke fill 0-3");
            GL.EnableVertexAttribArray(1);
            glnvg__checkError(gl, "stroke fill 0-4");
            int s = Marshal.SizeOf(typeof(NVGvertex));
            GL.VertexAttribPointer(0, 2, VertexAttribPointerType.Float, false, s, IntPtr.Zero);
            glnvg__checkError(gl, "stroke fill 0-5");
